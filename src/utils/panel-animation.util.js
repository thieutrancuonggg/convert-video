const { VIDEO_AREA_HEIGHT, PANEL_HEIGHT, TARGET_WIDTH, TARGET_HEIGHT } = require('../constants/video.constants');
...
// Panel-area positions stay in the four corners of the 460px panel.
function iconAnchor(pos) {
  const PAD    = 28;
  const panelY = VIDEO_AREA_HEIGHT; // 1460
...
function buildShineFilters(shineCount, duration) {
  ...
  return spreadTimes(shineCount, duration, startPad, endPad).flatMap((tBase) => {
    const filters = [];
    for (let i = 0; i < STRIP_COUNT; i++) {
      ...
      const isMid  = i >= Math.floor(STRIP_COUNT * 0.2) && i < Math.ceil(STRIP_COUNT * 0.8);
...
function buildIconFilters(icons, duration, fontOpt) {
  ...
  const startPad = Math.min(4, duration * 0.10);
  const endPad   = Math.min(6, duration * 0.10);
